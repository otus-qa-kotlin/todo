...
class TasksRepositoryMemoryTest : BehaviorSpec({

    given("empty repository and task") {
        ...
        `when`("add new task to repository") {
            ...
            then("it should be present in repository") {
    ...
    given("Task repository with not completed task") {
        val task = Task(1, "Task to complete", Priority.MEDIUM, false)
        val taskTestRepository = TasksRepositoryMemory()
        taskTestRepository.addTask(task)
    ...
    given("Task repository with completed task") {
        val task = Task(1, "Task to uncomplete", Priority.LOW, true)
        val taskTestRepository = TasksRepositoryMemory()
        taskTestRepository.addTask(task)
